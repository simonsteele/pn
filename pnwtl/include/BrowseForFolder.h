...
class CCustomBrowseForFolder
{
public:
	CCustomBrowseForFolder(const HWND hParent = NULL, const LPITEMIDLIST pidl = NULL, const int nTitleID = 0);
	CCustomBrowseForFolder(const HWND hParent, const LPITEMIDLIST pidl, const CString& strTitle);
	virtual ~CCustomBrowseForFolder() = 0;
...
inline UINT CCustomBrowseForFolder::GetFlags() const
{
	return m_bi.ulFlags;
}

inline int CCustomBrowseForFolder::GetImage() const
{
	return m_bi.iImage;
}

class CBrowseForFolder : public CCustomBrowseForFolder
{
	public:
		CBrowseForFolder(const HWND hParent = NULL, LPCTSTR title = NULL);
		virtual ~CBrowseForFolder();

		void SetInitialSelection(LPCTSTR Path);

		void OnInit() const;
		void OnSelChanged(const LPITEMIDLIST pidl) const;

	private:
		CString m_csInitialSelection;
};
