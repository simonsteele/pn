...
class JumpToPlugin : Plugin
{
	public:
		...
		bool GetMethods(const wchar_t* filename, HWND editorWnd, FP_CALLBACK callback, int mask, const wchar_t* scheme, LPVOID cookie);

	protected:
		typedef bool (__stdcall *LPFnGetMethods)(const wchar_t* filename, HWND editorWnd, FP_CALLBACK callback, int mask, const wchar_t* scheme, LPVOID cookie);
